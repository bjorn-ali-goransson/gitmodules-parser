...
* ```$submodule->gitmodules_path``` is the path to the .gitmodules file where this submodule is mapped. Used for nested submodules, and translating ```$submodule->path``` to a real path.
...
* ```$submodule->file_exists``` is true if the directory to the submodule exists.
